...
- ✅ **Fait (2026-07-30)** — Page d'accueil réduite à la seule liste des stages : `Home/Index.cshtml` ne garde plus que le panneau « Activités récentes » (les 4 cartes stats, le panneau « Inscriptions récentes » et les 5 boutons « Actions rapides » ont été retirés, conformément à la capture `18.08.04` annotée). `HomeController.Index()` simplifié en conséquence (ne charge plus parents/enfants/team members/bookings, juste les 5 activités les plus récentes) ; `DashboardViewModel` réduit à `RecentActivities`. La barre latérale (hamburger) elle-même n'est pas touchée — voir question ouverte n°6, toujours en attente de confirmation sur Contacts/Import/Équipe avant d'y toucher.
...
- ✅ **Fait (2026-07-30)** — Formulaire « Créer/Modifier une excursion » (capture `19.22.35` annotée) : **Heure de début**, **Heure de fin** et **Type** retirés de l'écran (devenus des champs `<input type="hidden">` qui préservent la valeur existante à l'édition, plutôt que supprimés du modèle). Nom, Description, Date, Coût et Groupes cibles restent visibles/éditables. `CreateExcursionViewModel.Type` a maintenant un défaut explicite (`ExcursionType.Other`) puisqu'il n'est plus choisi à la création.
- ✅ **Fait (2026-07-30)** — Liste « Gérer les excursions » (capture `19.25.31` annotée) : colonnes **Type d'excursion** et **Coût par enfant/Revenus totaux/Dépenses totales/Solde net** retirées ; ne restent que Nom/Date/Groupes ciblés/Inscriptions/Actions.
- Ne pas supprimer les champs actuels du modèle — les conserver en base pour une réintroduction ultérieure (Thomas prévoit de les redemander). ✅ respecté ci-dessus (champs cachés, pas supprimés).
...
- ✅ **Fait (2026-07-30)** — Alléger la vue principale (capture `19.34.30` annotée) : `TeamMembers.cshtml` passe d'une mise en page 2 colonnes (Équipe assignée / Membres disponibles côte à côte) à un empilement vertical — Équipe assignée en pleine largeur en premier, puis le panneau **« Membres disponibles »** en dessous, replié par défaut (accordéon Bootstrap, clic pour dérouler). Fonctionnalité d'ajout intacte, juste moins visible par défaut.
