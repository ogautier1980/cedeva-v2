...
Cedeva compte **~1254 tests** unit/intégration (+ **65** E2E navigateur, 0 `[Skip]`, et **3**
PostgreSQL) répartis sur **5 niveaux**, exécutés par **3 projets de test** et **3 workflows CI**.
...
graph TD
    ...
    sql["Intégration PostgreSQL (Testcontainers + Docker)<br/>traduction LINQ (ToLower), migrations réelles"]
...
de **haute fidélité** (vrai PostgreSQL, vrai navigateur) plus lents, isolés dans des workflows CI
...
| `tests/Cedeva.Tests.Sql` | 4 (PostgreSQL réel via Testcontainers) | nécessite Docker | workflow dédié `integration-sql.yml` |
...
- **Couverture :** tous les contrôleurs (Activities, Bookings, Children, Parents, TeamMembers,
  ...
  Home, AddressApi).
- **Infra :** `CedevaWebApplicationFactory` — démarre l'app réelle, remplace PostgreSQL par SQLite
...
### 4. Tests d'intégration PostgreSQL (Testcontainers)
...
- **Infra :** `PostgreSqlFixture` démarre un conteneur **PostgreSQL 17** (image `postgres:17-alpine`)
...
- Choix du niveau : logique métier → unitaire ; touche la DB → SQLite ; flux HTTP/auth/vue →
  WebApplicationFactory ; spécificité PostgreSQL (traduction ToLower) → Testcontainers ;
...
- Les workflows E2E et PostgreSQL tournent en parallèle mais **ne bloquent pas** le déploiement
...
- Intégrations externes réelles (envoi réseau réel Brevo, Stripe) : mockées, pas appelées.
